...
const App = () => {
    ...
    const [{ canDrop, isOver }, drop] = useDrop(() => ({
        ...
        drop(item) {
            if (item.files.length == 1 && item.files[0].type == "application/json") {
                readFile(item.files[0]).then(JSON.parse).then(p => setPlaylist(p));
                return;
            }
            setPlaylist(p => update(p, { tracks: { $push: item.files.map(createTrack) } }))
        },
        collect(monitor) {
    ...
    const setCsv = async (track, csv) => {
        ...
        const csvContent = csvContentStr.split("\r\n").map(l => l.split(",").map(Number)).map(([time, dir, val]) => [time * 0.1, dir, val])
        setPlaylist((p) => {
    ...
    return <div>
        ...
        <a download={`${playlist.name}.json`} href={playlistUrl}>save</a>
        <div ref={drop}
...
render(<DndProvider backend={HTML5Backend} ><App /></DndProvider>, document.getElementById("root"))