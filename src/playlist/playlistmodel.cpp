...
int PlaylistModel::columnCount(const QModelIndex &parent) const { return 4; };
...
QUrl PlaylistModel::getUrl(const QModelIndex &index) const {
  ...
  connect(meta_reader, &QMediaPlayer::mediaStatusChanged, [=] {
    ...
    if (status == QMediaPlayer::LoadedMedia) {
      ...
      m_data.append(PlayListItem{
          url, title, album, artist, trackNumber, duration, mediaType});
      endInsertRows();
      delete meta_reader;
    }
